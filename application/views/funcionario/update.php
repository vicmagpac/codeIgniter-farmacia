...
<body id="top">
	<div id="container">
		...
		<?php
			...
			$query = $this->funcionario_model->get_byid($idFunc)->row();
		?>
		...
		<div id="main-content">
			...
			<?php endif; ?>
			<form id="validate-form" class="block-content form" action="<?php echo site_url('index.php/funcionario/update/'.$idFunc); ?>" method="post">
				<input type="hidden" name="id" value="<?php echo $query->id_user; ?>" />
				<div class="_100">
					<p><label for="titulo">Nome</label><input id="nome" name="nome" class="required" required type="text" value="<?php echo $query->nome_user ?>" /></p>
				</div>
				<div class="_100">
					<p><label for="descricao">CPF</label><input id="cpf" name="cpf" class="required" required type="text" value="<?php echo $query->cpf ?>"  /></p>
				</div>
				<div class="_100">
					<p><label for="descricao">Telefone</label><input id="telefone" name="telefone" class="required" required type="text"  value="<?php echo $query->telefone ?>" /></p>
				</div>
				<div class="_100">
					<p>
						...
						<select name="cargo" required>
							...
							<option <?php echo $query->id_cargo == 1 ? 'selected="selected"' : '' ?> value="1">Gerente</option>
							<option <?php echo $query->id_cargo == 2 ? 'selected="selected"' : '' ?> value="2">Vendedor</option>
						</select>
					</p>
				</div>
				<div class="_100">
					<p>
						...
						<select name="sexo" required>
							...
							<option  <?php echo $query->sexo == 'M' ? 'selected="selected"' : '' ?>  value="M">Masculino</option>
							<option  <?php echo $query->sexo == 'F' ? 'selected="selected"' : '' ?> value="F">Feminino</option>
						</select>
					</p>
				</div>
				<div class="_100">
					<p><label for="descricao">Rua</label><input id="rua" name="rua" class="required" required type="text" value="<?php echo $query->rua ?>" /></p>
				</div>

				<div class="_100">
					<p><label for="descricao">Número</label><input id="numero" name="numero" class="required" required type="text" value="<?php echo $query->numero ?>" /></p>
				</div>

				<div class="_100">
					<p><label for="descricao">Bairro</label><input id="bairro" name="bairro" class="required" required type="text" value="<?php echo $query->bairro ?>" /></p>
				</div>

				<div class="_100">
					<p><label for="descricao">Complemento</label><input id="complemento" name="complemento" type="text"  value="<?php echo $query->complemento ?>"/></p>
				</div>
				<div class="_100">
					<p>
						...
						<select name="cidade" required>
							...
							<?php foreach($cidades as $cid): ?>
								<option <?php echo $query->id_cidade == $cid->id ? 'selected="selected"' : '' ?> value="<?php echo $cid->id ?>"><?php echo $cid->nome ?></option>
							<?php endforeach; ?>
						</select>
					</p>
				</div>
				...
				<div class="block-actions">
					...
					<ul class="actions-right">
						<li><input type="submit" class="button" value="Editar"/></li>
					</ul>
				</div>
			</form>
