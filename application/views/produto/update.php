...
<body id="top">
	<div id="container">
		...
		<?php
			...
			$query = $this->produto_model->get_byid($idProd)->row();
		?>
		...
		<div id="main-content">
			...
			<?php endif; ?>
			<form id="validate-form" class="block-content form" action="<?php echo site_url('index.php/produto/update/'.$idProd); ?>" method="post">
				<input type="hidden" name="id" value="<?php echo $idProd; ?>" />
				<div class="_100">
					<p><label for="titulo">Nome</label><input id="nome" name="nome" class="required" required type="text" value="<?php echo $query->nome ?>" /></p>
				</div>
				<div class="_100">
					<p>
						<label for="alvo">Tipo</label>
						<select name="tipo" required>
							<option value="">Selecione o tipo de Medicamento</option>
							<option <?php echo $query->tipo == 'MARCA' ? 'selected="selected"' : '' ?> value="MARCA">Marca</option>
							<option <?php echo $query->tipo == 'GENÉRICOS' ? 'selected="selected"' : '' ?> value="GENÉRICOS">Genérico</option>
							<option <?php echo $query->tipo == 'SIMILARES' ? 'selected="selected"' : '' ?> value="SIMILARES">Similares</option>
						</select>
					</p>
				</div>
				
				<div class="_100">
					<p><label for="descricao">Valor</label><input id="valor" name="valor" class="required" required type="text"  value="<?php echo $query->valor ?>" /></p>
				</div>

				<div class="_100">
					<p><label for="descricao">Quantidade</label><input id="quantidade" name="quantidade" class="required" required type="text"  value="<?php echo $query->quantidade ?>" /></p>
				</div>
				<div class="_100">
					<p>
						<label for="alvo">Fornecedor</label>
						<select name="fornecedor" required>
							<option value="">Selecione o Fornecedor</option>
							<?php foreach($fornecedores as $for): ?>
								<option <?php echo $query->id_fornecedor == $for->id ? 'selected="selected"' : '' ?> value="<?php echo $for->id ?>"><?php echo $for->nome ?></option>
							<?php endforeach; ?>
						</select>
					</p>
				</div>
